...
# -------------------------------------------------
# 1. Load model & feature list once
# -------------------------------------------------
@st.cache_resource
def load_model():
...
if st.button('Run Prediction', type="primary"):
    ...
    try:
        ...
        # TreeExplainer on binary classifiers returns a list [class0, class1]
        ...
        st.warning(f"SHAP explanation could not be rendered: {e}")
        st.info("Tip: Ensure `shap` is installed (`pip install shap`) and the model is tree-based.")
